...
```
## Visualize clusters just created
```{r}
cve_bin_long_clusts <- left_join(cve_bin_long, cve_version_clusters)
```

Step 2. Add Binary attribute to long format
```{r}
alt_names = factor(c("small", "mid", "large"), levels=c("large", "mid", "small"))
...
cve_bin_long_clusts %>% ggplot( mapping = aes(x = version, y = vuln_count)) +
  geom_line(mapping = aes(group = filename, color = cluster_1.0), size = 2, alpha = 0.5) +
...
cve_bin_long_clusts %>% ggplot( mapping = aes(x = version, y = vuln_count)) +
  geom_line(mapping = aes(group = filename, color = cluster_2.1), size = 2, alpha = 0.5) +
  scale_x_discrete(guide = guide_axis(angle = 90)) +
  xlab("Version") +
  facet_grid(cols = vars(cluster_2.1)) +
  theme(legend.position = "top") +
  scale_fill_brewer(type = "qual")
