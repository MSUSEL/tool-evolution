...
# set seed
# Unix time stamp from while writing this code 132414231234
# set seed based on last 9 digits
set.seed(414231234)
...
scores_long[["cve_bin_tool"]] <- select(scores_long[["cve_bin_tool"]], -c(X, date))
names(scores_long[["cwe_checker"]])[names(scores_long[["cwe_checker"]]) == "findings_count"] <- "vuln_count"
...
cluster_names <- data.frame(
  ...
  cluster_title = c("high", "medium", "low", "medium", "low", "high")
)
